...
class Agent:
    ...
    def __init__(self, position, index, gender, age, seniority, fire, seniority_position, skills, id):
...
def create_company(titles:list, n:list):
    ...
    for i in range(len(titles)):
        key = titles[i] # level in company
        value = [None for i in range(0, n[i])]  # id for agents
        company[key] = value # company level?

    return company

def get_truncated_normal(mean=0, sd=1, low=20, upp=68): # agent age function
...
def skill_by_gender(company):
    '''
    Finds the total skill for each gender at each level - adds together skill for all women and skill
    together for all men

    Parameters
    ---------- 
    company : dictionary, a dictionary with the job titles as keys and empty lists as values
    '''
    female_skills = np.repeat(0, len(company.keys()))
    male_skills = np.repeat(0, len(company.keys()))

    for index, i in enumerate(company.keys()):
        for j in range(0, len(company[i])):
            if company[i][j] is not None:
                if company[i][j].gender == ['female']:
                    female_skills[index] += company[i][j].skills 
                else:
                    male_skills[index] += company[i][j].skills
    return female_skills, male_skills

def skill_list_by_gender(company): #driller
    '''
    Attempt to create list of skill by gender, ie. to create skill distributions at a given level
    ...
    for index, i in enumerate(company.keys()):
        for j in range(0, len(company[i])):
            if company[i][j] is not None:
                if company[i][j].gender == ['female']:
                    female_skills.append(company[i][j].skills)
                else:
                    male_skills.append(company[i][j].skills)
    return female_skills, male_skills

def mean_skills(company):
    '''
    Calculates the mean seniority of the company
    '''
    means = []
    for i in (company.keys()):
        skills_list = [i.skills for i in company[i] if i is not None]

        means.append(np.mean(skills_list))
...
def mean_seniority(company):
    ...
    for i in (company.keys()):
        ...
        means.append(np.mean(zipped_list)) # total senority, get mean 
...
def plot_gender(company, tick):
    ...
    tick : int, the current month (tick)
    '''
    ...
    n_jobs = len(labels)
    ncols = 3
    nrows = (n_jobs // ncols) + (n_jobs % ncols > 0)
    ...
    for i, jobtitle in enumerate(labels):
        # Bar chart
        ax = axs[i]
        rects1 = ax.bar(x[i] - width/2, female[i], width, label='Female', color='skyblue')
        rects2 = ax.bar(x[i] + width/2, male[i], width, label='Male', color='steelblue')

        # Add some text for labels, title and custom x-axis tick labels, etc.
        ax.set_ylabel('Count')
        ax.set_title('{} (month = {})'.format(jobtitle, tick + 1))
        ax.set_xticks([x[i]])
        ax.set_xticklabels([jobtitle])
        ax.legend()
    ...
    for i, jobtitle in enumerate(labels):
        # Pie chart
        ax = axs[i]
        sizes = [female[i], male[i]]
        ax.pie(sizes, labels=['Female', 'Male'], colors=['skyblue', 'steelblue'], autopct='%1.1f%%', startangle=90)
        ax.set_title(jobtitle)
        ax.axis('equal')  # Equal aspect ratio, pie is drawn as a circle
...
def plot_gender_pie(company, tick): ############## doesn't work ##############
    '''
    Plots the current gender distribution in each job-title of the company
    ...
    ax.pie_label(rects1, padding=3)
    ax.pie_label(rects2, padding=3)
...
def plot_gender_skills(company, tick): 
    '''
    Plots the current skill distribution in each job-title of the company
    ...
    tick : 
    '''
    female, male = skill_by_gender(company)
    f_count, m_count = count_gender(company)
...
def plot_gender_skills_violin(company, tick): ####### doesn't work maybe #######
    '''
    Plots the current skill distribution in each job-title of the company using violin plots
    ...
    tick : int, the current month (tick)
    '''
    female_skills, male_skills = skill_list_by_gender(company)
    # Find the maximum length among both lists
    max_len = max(len(female_skills), len(male_skills))

    # Pad the shorter list with NaN values to make both lists of equal length
    female_skills += [np.nan] * (max_len - len(female_skills))
    male_skills += [np.nan] * (max_len - len(male_skills))

    # Convert skills to a long-form DataFrame
    data = pd.DataFrame({
        'Gender': ['Female'] * len(female_skills) + ['Male'] * len(male_skills),
        'Skill': np.concatenate([female_skills, male_skills]),
        'Job Title': list(company.keys()) * 2
    })

    # Create violin plots
    fig, ax = plt.subplots(figsize=(12, 8))
    sns.violinplot(x='Job Title', y='Skill', hue='Gender', data=data, split=True, palette={'Female': 'skyblue', 'Male': 'steelblue'}, ax=ax)

    # Add some text for labels, title and custom x-axis tick labels, etc.
    ax.set_ylabel('Skill Level')
    ax.set_title(f'Skill Distribution by Job Title and Gender, Month = {tick + 1}')
...
# function for plotting the gender skill distribution development over all ticks
def plot_gender_skills_development(company: dict, months:int, data: pd.DataFrame):
    '''
    Plots the skills of each gender in each job-title in the company for each tick
...
def promote_agent(company:dict, i, j, ind_i, weight:dict, bias: list, threshold: list, diversity_bias_scaler: float, id, intervention, mean_senior, mean_sk):
    ...
    diversity_bias = 0 #diversity_check(gender_distribution, ind_i, threshold, diversity_bias_scaler)
    choose_agent_promotion(company, i, j, ind_i, weight, bias, diversity_bias, id, intervention, mean_senior, mean_sk)

def choose_agent_promotion(company, i, j, ind_i, weight, bias, diversity_bias, id, intervention, mean_senior, mean_sk):
    '''
  ...
  # ADD SKILL TO THE MEAN AAAA - CE 
        ...
        for k in range(0, len(company[index])):
            if company[index][k] is not None:
                if company[index][k].gender[0] == 'male' and intervention != 'blinding':
                    ...
                    add_bias = 0
                weights.append((company[index][k].seniority_position + company[index][k].seniority)/(mean_senior[ind_i+1])+(company[index][k].skills - mean_sk[ind_i+1])/(mean_sk[ind_i+1]) + add_bias) # ADDD SKILL TO THE MIX - CE, seniority deleted, skill added
                genders.append(company[index][k].gender[0])
            else:
                weights.append(None)
                genders.append(None)
        ...
        if intervention == 'quota_list':
            # Number of candidates to select
            n_candidates = -weight[i]['candidates']
    
            # Process weights to handle None values by replacing them with a very negative number (-1e10)
            #processed_weights = np.where(~np.isin(weights, [None]), weights, -1e10)
    
            # Sort indices based on processed weights in descending order
            #sorted_indices = np.argsort(processed_weights)[::-1]
            #sorted_genders = [x for _,x in sorted(zip(genders, processed_weights))]

            #female_indices = []  # List to store selected indices
            #male_indices = []  # Counter for male candidates
            #female_count = 0  # Counter for female candidates

            # Iterate over the sorted indices to select candidates based on gender quota
            #i = 0
            #for index in sorted_indices:
                #if len(female_indices) + len(male_indices) >= (-n_candidates):
                    #break

                #elif len(female_indices) >= (-n_candidates/2) and genders[i] == 'female':
                    #female_indices.append(index)
                # Select male candidates if conditions are met
                #elif len(male_indices) >= (-n_candidates/2) and genders[i] == 'male':
                    #male_indices.append(index)

                #i += 1
                    # Filter weights to include only those of the promotion candidates
                

            # get the index for sorting the weights in descending order
            processed_weights = np.where(~np.isin(weights, [None]), weights, -1e10)

            sorted_index = np.argsort(processed_weights)[::-1]
            #print('genders', len(genders))
            #print(len(weights))
            #print(len(processed_weights))
            #print(len(sorted_index))

            #print('genders', genders)
            #print(weights)
            #print(processed_weights)
            #print(sorted_index)



            # sort the genders by the index
            gender_sorted = np.array(genders)[sorted_index]


            # get the indices of the first 2 males and first two females of the sorted
            female_best_candiates = np.where(gender_sorted == "female")[0][:int(n_candidates/2)]
            males_best_candidates = np.where(gender_sorted == "male")[0][:int(n_candidates/2)]

            # get the indices of the best candidates
            best_candidates = np.concatenate([female_best_candiates, males_best_candidates])


            # get the original indices of the best candidates before sorting
            promotion_candidates_index = sorted_index[best_candidates]

#print(original_indices)

            #promotion_candidates_index = female_indices.append(male_indices)
        ...
        #print('n_', (-n_candidates), (-n_candidates/2))
        #print('males', male_indices)
        #print('females', female_indices)
        #print('genders', genders)
        #print("n_candidates", n_candidates)
        #print("sorted", sorted_indices)
        #print("Promotion candidates index:", promotion_candidates_index)
        #print("Weights:", weights)
        #print("Company Level:", company[index])
        #print("Index Keys:", list(company[index].keys()))
        ...
        #print("Promoted Agent:", agent.index )

        # setting the seniority in the position as 0 - happens after promotion (reset)
...
def update_agents(company, i, j, weight:dict, months_pl, intervention):
    ...
    if intervention == None:
        if company[i][j].gender[0] == 'female':
            update_parental_leave(company, i, j, months_pl, intervention)
...
def update_parental_leave(company: dict, i, j, months_pl, intervention):
    '''
    ...
    if company[i][j].parental_leave != None:
        ...
        parental_leave(company, i, j, intervention)

def parental_leave(company, i, j, intervention):
    '''
    ...
    if intervention != 'shared_parental':
        if company[i][j].age >= 20 and company[i][j].age <= 24 and company[i][j].gender == 'female':
            leave = random.choices([1, None], weights = [0.072/(12*5), 0.9988])
            company[i][j].parental_leave = leave[0]
        elif company[i][j].age >= 25 and company[i][j].age <= 29 and company[i][j].gender == 'female':
            leave = random.choices([1, None], weights = [0.0981/(12*5), 0.998365])
            company[i][j].parental_leave = leave[0]
        elif company[i][j].age >= 30 and company[i][j].age <= 34 and company[i][j].gender == 'female':
            leave = random.choices([1, None], weights = [0.1005/(12*5), 0.998325])
            company[i][j].parental_leave = leave[0]
        elif company[i][j].age >= 35 and company[i][j].age <= 39 and company[i][j].gender == 'female':
            leave = random.choices([1, None], weights = [0.0524/(12*5), 0.9988])
            company[i][j].parental_leave = leave[0]
        elif company[i][j].age >= 40 and company[i][j].age <= 45 and company[i][j].gender == 'female':
...
def diversity_check(gender_distribution, ind_i, threshold, diversity_bias_scaler):
    ...
    #else:
    diversity_bias = 0
        #(percent_women - threshold) * diversity_bias_scaler # er den her ikke omvendt af hvad den skal?
    #diversity_bias = 0 #no effect ceac
...
def run_abm(months: int, save_path: str, company_titles: list, titles_n: list, weights: dict, bias_scaler: float = 1.0, plot_each_tick = False, months_pl: float = 9, threshold: float = 0.3, diversity_bias_scaler: float = 1.0, intervention = None, company = None):
    ...
    data = create_dataframe(['tick', 'gender', 'skills'], company_titles)
    adata = pd.DataFrame()
    ...
    for month in range(months):
        ...
        for ind_i, i in enumerate(company.keys()):
            for j in range(0, len(company[i])):
                ...
                if company[i][j] == None:
                    start = time()
                    promote_agent(company, i, j, ind_i, weight=weights, bias = bias, threshold = threshold[ind_i], diversity_bias_scaler = diversity_bias_scaler, id = id, intervention = intervention, mean_senior = mean_senior, mean_sk = mean_sk)
                    end = time()

                    promoting_time += end - start
                
                start = time()
                dat = {'id': company[i][j].id, 'gender': company[i][j].gender[0], 'age': company[i][j].age, 'seniority': company[i][j].seniority, 'seniority_pos': company[i][j].seniority_position, 'parental_leave': company[i][j].parental_leave, 'position': company[i][j].position, 'skills': company[i][j].skills, 'tick': month}
                adata_list.append(dat) #becomes a list, faster than creating a dataframe (concat) as below
        ...
        if plot_each_tick:
            ...
            plot_gender_skills(company, tick = month)
        
        counts = count_gender(company)
        skills = skill_by_gender(company)
        ...
        f_skills = {'gender': 'female', 'tick': month, 'Level 1': skills[0][0],'Level 2': skills[0][1], 'Level 3': skills[0][2], 'Level 4': skills[0][3], 'Level 5': skills[0][4], 'Level 6': skills[0][5]}
        m_skills = {'gender': 'male', 'tick': month, 'Level 1': skills[1][0],'Level 2': skills[1][1], 'Level 3': skills[1][2], 'Level 4': skills[1][3], 'Level 5': skills[1][4], 'Level 6': skills[1][5] }
        ...
        # create pandas dataframe from dictionaries f and m
        #skills_data = pd.DataFrame.from_dict([f_skills, m_skills]) - driller
        #data = pd.concat([data, skills_data], ignore_index=True, verify_integrity=False, sort=False)
        end = time()

        concatenate_time += end - start

        print('concatenate_time: ', concatenate_time)

        print('tick {} done'.format(month))

    # plotting the gender development over time
    plot_gender_pie # virker ik 
    plot_gender_development(company, months = months, data = data)
    plot_gender_skills_development(company, months = months, data = data)
    plot_gender(company, tick=month)
